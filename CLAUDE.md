...
| 路径 | 说明 |
|------|------|
| `main.go` / `cli.go` / `render.go` | 单一入口 `aibalance`（位于仓库根目录，可直接 `go install .`）：默认启动 bubbletea + lipgloss 终端仪表盘（内嵌抓取与 Chrome 启动器，`r` 刷新、`l` 为需要登录的服务打开登录页、`q` 退出、`--once` 单次模式；卡片顶边框右侧是该服务的刷新状态位——正在刷新显示灰色 `⟳`，否则显示距上次刷新的耗时，并按已用刷新间隔比例着色：<30% 绿、30–80% 黄、≥80% 红（自动刷新始终开启，无总开关）；无卡片的异常/需要登录行在刷新时把 detail 换成 `⟳ refreshing`，状态栏不再显示刷新进度；各卡片独立刷新——每服务一个独立 Cmd/context/超时，完成即合并显示与缓存，自动刷新按服务独立到期触发，`r` 跳过在飞服务；CLI 子命令保持串行）；`cli` 子命令提供 Python 兼容 CLI（`--only` / `--json` / `--progress-jsonl` / `--debug` / `--strict-exit`）；`config` 子命令（`config.go`）用 stdin 菜单交互编辑 `config.json`（`<n>` 切换开关、`<n> <秒>` 设间隔、`s` 保存、`q` 退出——有未保存修改时需再按一次 `q` 确认丢弃，EOF 丢弃时向 stderr 打提示，保存走 `SaveGUISettings` 原子写；`--edit` 改用 `$EDITOR`（默认 notepad）直接打开文件，退出后校验 JSON）；主入口 `-h` 由 `printMainUsage` 列出子命令。 |
| `internal/aibalance/` | 抓取核心包（见下）。 |
| `config/` | 配置原型目录：`gui_settings.json.example` 由 `embed.go` 以 `go:embed` 编译期嵌入 exe；用户目录缺文件时加载器把它物化成实体文件，example 即默认值单一来源（代码常量仅兜底，`deepseek_api_key` 在模板中必须保持空串惰性，由 `TestEmbeddedGUISettingsExampleMatchesRegistry` 强制）。服务默认全部 `enabled: false`（opt-in：用户在 config 子命令中启用后才参与刷新与显示，缺省/未列出条目的兜底同样为禁用；仅影响 TUI，cli 子命令不受 enabled 过滤）。文件格式为两层 `{meta: {version: "2"}, fields: {...}}`，由 go-config-manager v0.6.0 管理。 |
...
| `browser.go` | CDP 连接（loopback 校验）、每服务专属常驻 tab（`acquireServicePage`，`claimServiceTarget` 按 origin 精确认领、匹配不到再按注册域（host 最后两段）回退，避免未登录重定向到同域兄弟子域后每轮刷新新建 tab）、`probeWebDashboard`（所有等待带超时封顶、同文档导航走 ignore-cache reload）、`makeWebDashboardRunner`、`OpenLoginPages`（TUI 按 `l` 打开登录页，复用该服务已有 tab 并置前，不新建）。 |
...
| `deepseek.go` / `zai.go` / `qwen.go` / `kimi.go` / `qoder.go` / `codex.go` / `tencent.go` / `openrouter.go` | 各服务的抓取与解析（`summarize*` 纯函数）。 |
...
| `cache.go` / `envfile.go` / `guisettings.go` / `startup.go` | `latest_summary.json` 原子读写（复用 `writeUserDataFile`）、用户数据目录定位（`UserDataDirectory` 供缓存与遗留文件，`UserConfigDirectory` 供 config.json）与 `.env.local` 宽松解析（`parseEnvLocal` 纯函数）、`config.json` 加载/保存（通过 go-config-manager v0.6.0 的 Manager 实例模型，`assembleConfigManager` 统一装配 Init/RegisterDefaults/SetCurrentVersion 与升级链；缺文件时物化嵌入 example；坏文件返回 `*configmanager.CorruptConfigError`，所有入口 fatal 退出；v1 与无版本号文档在 Load 时被版本链自动晋升为 v2 并回写磁盘，声明版本无法到达 v2 的文件走"警告后用 example 兜底"路径；enabled 过滤（服务 opt-in，默认禁用）、自动刷新间隔（自动刷新无总开关、始终开启；旧文件残留的 `auto_refresh` 键被解码器忽略，下次保存时自然清除）、环境字段）、`LoadStartupSettings` 统一启动入口（物化 + 一次性 `.env.local` 迁移 + 把环境字段桥接为进程环境变量，真实环境变量不被覆盖）。 |
...
- 每个浏览器服务在各自 Chrome 里有一个专属常驻 tab（`claimServiceTarget` 按 origin 精确认领，匹配不到再按注册域（host 最后两段，如 `chat.z.ai` → `z.ai`、`cloud.tencent.com` → `console.cloud.tencent.com`）回退认领，仍找不到才新建且不关闭）。Chrome 启动默认开的 New Tab 页在首个服务 tab 认领成功后由 claim 顺手关闭（`closeNewTabTargets`）——不能在启动后立刻关：有头 Chrome 关掉最后一个 tab 会整个进程退出，必须等有服务 tab 撑着再关。注册域回退是必须的：未登录时站点会把 tab 重定向到同域兄弟子域的登录/授权页（z.ai → `chat.z.ai/auth`、腾讯云控制台 → `cloud.tencent.com/login`），纯 origin 匹配认领不到，会导致每轮刷新新建一个 tab 无限累积。前提假设：每个自动化 Chrome 内同注册域只有一个服务（当前成立）。`OpenLoginPages`（`l` 键）同样走认领复用已有 tab 并置前，不无条件新建。tab 已在目标页面时 `probeWebDashboard` 用 ignore-cache reload，避免同 URL 导航命中缓存恢复而不触发 API；其余情况正常导航。probe 内所有等待（导航事件、load、数据就绪、Eval、响应 body 获取、profile 菜单收集）都有超时封顶，页面持续轮询或有挂起请求时不会卡满整个 pass。
- **不要用 rod 的 `WaitRequestIdle` 等 SPA 加载完成**：它的时长参数是"最短静默时长"而非超时上限，所以每个服务至少白等 10 秒；而阿里云百炼控制台这类持续轮询的页面永远凑不出静默窗口，只能把 `TimeoutMS` 预算整个烧光。`waitForDashboardData` 改为等各服务在 summarizer 旁声明的必需 API 响应（`qwenRequiredResponses` 等）落地即走，并以"抓取计数停止增长 `captureQuietWindow`"兜底——同一 summarizer 的不同 host 未必调用全部接口（BigModel 没有 `model-usage`，z.ai 未开 usageBoard 时不发 `monitor/*`）。必需响应全部到手时跳过 `WaitMS` 静置延迟：那段延迟是为了等叫不出名字的迟到 XHR。兜底路径静默到期时还有一道**结果就绪判定**（`dashboardReadiness`，`evaluateReadiness`）：用该服务真实的 summarizer 在"当前已抓到的响应体（`collector.snapshot` 按 requestID 缓存 body）+ 当前页面文本"上试跑一次，若试跑 summary 已携带卡片显示的关键数据（各服务 `*SummaryReady` 谓词，如 z.ai 的 `five_hour`/`weekly`），同样跳过静置延迟直接出结果——必需集合永远凑不齐的服务（BigModel、z.ai 无 usageBoard）此前每轮都要白付 3s settle。判定时点仍是静默窗口到期，不会早于旧放弃点退出，故不损数据完整性。实测单次并发 pass：Qwen 33.1s→5.7s、Kimi 33.1s→2.6s、Qoder 20.3s→3.8s、BigModel 17.6s→7.9s、z.ai 30.3s→15.4s。
- **rod 的 `Element.Click` 会在后台隐藏 tab 挂死**：它经 `WaitInteractable` → `ScrollIntoView` → `WaitRepaint` 等 `requestAnimationFrame`，而隐藏 tab 永不派发 rAF；且 `WaitRepaint` 走 `p.root` 求值，绕过 `Page.Timeout` 克隆上的超时 ctx，于是一直挂到整个 pass ctx 死亡，之后所有 eval 立刻报 `context deadline exceeded`。需要点击时用带超时的 JS 点击代替：`el.Eval("() => this.click()")`（见 `clickFirstVisible`）。
- `chatgpt_codex` 声明了必需响应（`codexRequiredResponses`：`wham/usage`、`wham/usage/daily-token-usage-breakdown`、`wham/rate-limit-reset-credits`——analytics 页每次加载都会发这几个 XHR，到齐即走，不再走 `WaitRequestIdle`；chatgpt.com 永不网络静默，走 idle 会把每个候选的导航预算整个烧光）。chatgpt.com 的 JSON 响应体疑似经 service worker 提供，`Network.getResponseBody` 一律返回 -32000 No resource with given identifier found，故其用量解析完全依赖页面文本（`Weekly usage limit X% remaining` 等正则）。但 SPA 渲染晚于 XHR 到达（后台 tab 更慢），profile 收集器开头先由 `waitForCodexUsageText` 轮询到 summarizer 可解析的文本出现（`profileCollectorTimeout` 30s 封顶），否则空文本会连锁烧完 8 个候选 URL。它顺序尝试 8 个候选 URL，needs_login 时短路（同一 session 下后续候选只会重复跳登录页）。
...
- **配置文件损坏时所有入口（TUI、cli、config 子命令）统一打印含文件路径的错误并以非零码退出**，不再沿用旧的"警告后用默认值继续"行为。修复流程预留但尚未实现（库侧 `Manager.Repair` 为桩接口）。
